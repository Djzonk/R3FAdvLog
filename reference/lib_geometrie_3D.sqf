 ...
 * En: Library 3D Function Calculations In a (Intersection Calculations, rotations, etc.).
 *
 * Fournit une série de fonctions de calculs d'intersection (bounding sphere, bounding box, rayon-bbox)
 * En: Provides a series of intersection calculation functions (bounding sphere, bounding box, bbox-ray)
 * Fournit des fonctions de génération de matrices 3x3 de rotations yaw, pitch, roll
 * En: Provides functions that generate 3x3 matrices of yaw rotations, pitch, roll
 * Fournit des fonctions de multiplications vec3 x mat3x3 et mat3x3 x mat3x3
 * En: Provides functions multiplications vec3 x mat3x3 and mat3x3 x mat3x3
 *
...
/**
 * Compute the intersection of a ray with an object
...
/**
 * Retourne une position dégagée dans le ciel En:Return a clear position in the sky
 * @param 0 (optionnel) offset 3D du cube dans lequel chercher une position (défaut [0,0,0])
 * En:@param 0 (optional) offset 3D cube in which to look for a location (default [0,0,0])
 * @return position dégagée (sphère de 50m de rayon) dans le ciel
 * @return disengaged position (sphere of radius 50 m) into the sky
 */
R3F_LOG_FNCT_3D_tirer_position_degagee_ciel = {
	...
	// En: Find an open position (sphere of radius 50 m) into the sky
	for [
		{
